function AP_getConcatTrace_batch_momscope(animal,roi_path)
...
for curr_day = 1:length(days);
    ...
    tiff_loops = cellfun(@(x,y) str2num(x(y(2)+1:y(3)-1)), ...
    ...
    if ~processed_flag || (processed_flag && im_flag)
        ...
        framerate_indx = cell2mat(cellfun(@(x) ~isempty(strfind(x, ...
            'state.acq.frameRate')),img_parameter,'UniformOutput',0));
        ...
        warning('Check note: not sure whether to concat or not here');
