function AP_processJunData_bscope(animal)
% Adapted from AP_make_bgROI_batch(animal) (AP 2/13/14)
% Uses Jun's file/folder structure
%
% For use with upgraded momscope (same scanimage and properties as bscope,
% so just called bscope in function name)
%
...
%AP_sumMovie(50,animal,days,'Jun')

% NOTE: This step is not needed anymore: 50 summed movies are already
% created during Aki's queued motion correction script
...
for curr_day = 1:length(days)
    ...
    if length(roi_file) == 0
        warning([animal ' ' day ' has no ROI file, skipping']);
        continue
        % Sometimes ROIs aren't drawn purposely because of lost behavior
        % trials, because too many filled cells, etc. In the case that ROI
        % files are lacking, skip bg roi and only load in whatever's
        % available for processed file
    elseif length(roi_file) > 1;
        error([animal ' ' day ' has multiple ROI files'])
        % If multiple ROIs, then error out and fix
    end
    roi_filename = [day_path filesep roi_file.name];
    load(roi_filename,'-MAT');
    
    % load summed movie
    % Jun's filename structure for this is all over the place so there's a
    % bunch of wildcards
    im_dir = [data_path filesep animal '_' day];
...
AP_getConcatTrace_batch_jun_bscope(animal,[processed_dir filesep animal '_bgROI']);
