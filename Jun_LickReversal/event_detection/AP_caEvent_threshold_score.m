function AP_caEvent_threshold_score(data_all,mice,n_animals,n_sessions,n_events)
...
% Pick n animals at random
use_animals = randperm(length(mice),n_animals);

% Initialize the data from each event to extract
events = struct('trace',cell(total_events,1),'std',cell(total_events,1), ...
...
for curr_animal_idx = 1:n_animals
    ...
    for curr_session_idx = 1:n_sessions
        ...
        % Threshold data, pick n events at random to extract
        curr_concat_data = horzcat(data_all(curr_animal).im(curr_session).roi_trace_df{:});
        [caEventIdxs caEvents caStds] = AP_caEvents_threshtest(curr_concat_data,2);
        event_rois = cellfun(@(x,y) y*ones(size(x)),caStds,num2cell(1:length(caStds))','uni',false);
        event_num = cellfun(@(x) 1:length(x),caStds,'uni',false);
        event_ids = [horzcat(event_rois{:})' horzcat(event_num{:})'];
        ...
        for curr_event_idx = 1:n_events
            ...
            extract_sum_frames = unique(ceil(extract_frames/sum_frames));
            ...
            events(cumulative_events).std = caStds{curr_event_id(1)}(curr_event_id(2));
...
handles.active_trace_plot = plot((1:length(events(1).trace))/ ...
    events(1).sumframes,curr_activetrace, ...
    'r','linewidth',1,'parent',handles.trace_axes);
handles.inactive_trace_plot = plot((1:length(events(1).trace))/ ...
    events(1).sumframes,curr_inactivetrace, ...
...
if change_event
    ...
    set(gui_data.handles.active_trace_plot,'XData', ...
        (1:length(gui_data.events(new_event).trace))/ ...
        gui_data.events(new_event).sumframes, ...
...
if save_scores
    ...
    save_dir = '/usr/local/lab/People/Andy/Jun_lickreversal/event_detection/EventScoreDataDrop';
    
    % Set up strucure to save, package data to save
    event_scores = struct('std',cell(size(gui_data.events)), ...
        ...
        'score',cell(size(gui_data.events)));
    
    [event_scores.std] = gui_data.events(:).std;
