...
score_path = '/usr/local/lab/People/Andy/Jun_lickreversal/event_detection/EventScoreDataDrop';
...
tp_rate_scope = cellfun(@(x) x(:,1)./x(:,2),tp_scope,'uni',false);
fp_rate_scope = cellfun(@(x) x(:,1)./x(:,2),fp_scope,'uni',false);
...
ft_ratio_scope = cellfun(@(t,f) f(:,1)./(f(:,1)+t(:,1)), ...
...
ft_ratio_scope_combined = cellfun(@(te,ti,fe,fi) ...
    (fe(:,1)+fi(:,1))./(fe(:,1)+fi(:,1)+te(:,1)+ti(:,1)), ...
    tp_scope(:,1),tp_scope(:,2),fp_scope(:,1),fp_scope(:,2),'uni',false);
