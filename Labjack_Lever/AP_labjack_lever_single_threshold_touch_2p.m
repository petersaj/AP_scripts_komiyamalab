function AP_labjack_lever_single_threshold_touch_2p
...
%     try
        ...
        % This throws an error ONLY ON THIS COMPUTER AND ONLY IF ITS NOT
        % RUN FROM THE COMMAND LINE but I'm sick of dealing with this shit
        % and using the workaround they posted on the website
...
% Set default values
rest_threshold_default = -0.75; % V
press_threshold_default = -0.4; % V
threshold_time_default = 200; % ms
use_touch_default = true; % use touch sensor or not
...
% Set use touch and initial touch state
use_touch = use_touch_default;
touch_state = NaN;
...
% Create toggle button for touch requirement
touch_h = uicontrol('Style','togglebutton', ...
    'Position',[610 220 200 50],'String','Use touch sensor','FontSize',14, ...
    'Callback',@touch_callback,'Value',use_touch_default);
...
%% Callback for touch button
    function touch_callback(obj,event)
        use_touch = get(obj,'Value');
    end
...
%% Timer function for reading lever
    function getLever(obj,event)
        ...
        % Update touch state if crosses threshold
        % the ~ is because the touch sensor is high if no touch
        if use_touch
        ...
        if  successful_press && touch_state && ...
                (etime(clock,last_success_time) > refractory_period)
