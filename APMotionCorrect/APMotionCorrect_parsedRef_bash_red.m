...
imageinfo=imfinfo(filelist{end},'tiff');
M=imageinfo(1).Width;
N=imageinfo(1).Height;

disp('Creating Reference: NEW! Most stable 100 frames of last movie')
%ref_numframes = 0;
im_t = zeros(N,M);
...
refframes_channels = 2:channels:ref_numframes;
...
for ref_frame = 1:length(refframes_channels)
   ...
   curr_frame = double(imread(filelist{end},'tiff',curr_ref_frame));
