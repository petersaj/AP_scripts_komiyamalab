...
%         chone(i,:,:)=imread(fullfilename,'tiff',i);
    ...
    if numframes > 49
        ...
        gain=tk_find_gain(chone,25);%10.3
    else
        gain = 10.3;
    end
    %make sure there are no zeros .. ln(0) is bad
...
%     %chone=chone/gain;
    ...
    for i=1:numstills
        if i == numstills
            stillimages(i,:,:)=stillimages(i-1,:,:);
        else
            chone = zeros(framesper30sec,N,M);
            for j=1:framesper30sec
                chone(j,:,:)=imread(fullfilename,'tiff',framesper30sec*(i-1)+j);
            end
            chone(find(chone<1))=1;
%             chone=chone/gain;
            [minval,minimage]=min(mean(mean(abs(diff(chone(1:end,:,:),1)),2),3));
            stillimages(i,:,:)=chone(minimage,:,:);
        end
    end
    %%%%%%%%%%%%%%%%%
...
%     stillindex=floor(i/framesper30sec)+1;
    %TK090225: or use the reference frame in the middle
    stillindex = floor(numstills/2);
