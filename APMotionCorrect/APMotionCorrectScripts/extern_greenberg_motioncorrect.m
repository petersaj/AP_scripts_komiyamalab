function [p, iter_used, corr, failed, settings, xpixelposition, ypixelposition] = ...
  extern_greenberg_motioncorrect(data,base_image,subrec,settings,est_p)
%EXTERN_GREENBERG_MOTIONCORRECT
...
comp = 1 - frac;
 ...
 wb = waitbar(0, 'Processing via Greenberg et al. ...');
for j = 1:nframes    
    T = reshape(data(:,:,j),[],1);
    ...
    else %alignment failed -- sp: I still use what it got
%        blocksused = unique(blockind(mask));
        %zero unused parameters
%        p(j,1:min(blocksused) - 1) = 0;
%        p(j,(1:min(blocksused) - 1) + nblocks + 1) = 0;
...
% SP:
				disp(['extern_greenberg_motioncorrect.m::failure on frame ' num2str(j) '; using 0 as displacement.']);
				disp(['extern_greenberg_motioncorrect.m::try lowering your settings.corr_thresh - thresh failed at: ' num2str(corr(j))]);
				est_fromlast = repmat(0,1,2*(nblocks+1));
        p(j,:) = 0;        
	  end
	   waitbar(j/nframes,wb);
		%disp(['extern_greenberg_motioncorrect.m::finished frame ' num2str(j) ' of ' num2str(nframes)]);
end
 delete(wb);
...
%    corr = nan;
 %   p(:) = nan; %% SP : p(:) = best_p
    corr = 0;
    p(:) = 0; %% SP : p(:) = best_p
    disp('extern_greenberg_motioncorrect::warning - found no best fit so using 0 instead as dx/dy.');
else
    corr = best_corr;
    p = best_p; 
    iter = best_iter;
