function AP_motionCorrect_bscope_2ch_parallel(orig_dir, save_dir, channel_correct)
% AP_motionCorrect_bscope(orig dir, save dir, channel to correct by)
% Run turboreg without GUI
% 2ch version: This takes 2 channel inputs (green/red), motion corrects using 
% the chosen channel, applies offsets to both channels (not sub-pixel turboreg 
% for the non-chosen channel)
...
for session = 1:num_session
    ...
    if parsedfile == 1 && autoRef == 1;
        APMotionCorrect_parsedRef_bash_2ch
    end
    % run turboreg
    APMotionCorrect_turboreg_bash_2ch_parallel
    ...
    % copy over any non-tiff files and folders (i.e. for xsg, bhv)    
    dir_currfolder = dir(orig_dir);
    dir_filenames = {dir_currfolder.name};
    % exclude '.' and '..' directory markers
    dir_filenames = dir_filenames(3:end);
    tiff_files = cellfun(@(x) ~isempty(strfind(x,'.tif')), dir_filenames);
    copy_files = dir_filenames(~tiff_files);
    for copy_file_idx = 1:length(copy_files);
        copyfile([orig_dir filesep copy_files{copy_file_idx}], ...
                [save_dir filesep copy_files{copy_file_idx}]);
    end
