function AP_motionCorrect_bscope_red(orig_dir, save_dir)
% AP_motionCorrect_bscope(orig_dir, save_dir)
% Run turboreg without GUI
% Red version: This takes 2 channel inputs (green/red), motion corrects by the red,
% applies offsets to both channels. 
% NOTE: ASSUMES RED IS CHANNEL 2
...
for session = 1:num_session
    ...
    if parsedfile == 1 && autoRef == 1;
        APMotionCorrect_parsedRef_bash_red
    end
    % run turboreg
    APMotionCorrect_turboreg_bash_red
