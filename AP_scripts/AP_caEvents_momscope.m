function caEvent_matrix = AP_caEvents_momscope(df_matrix,pyr_cells,gad_cells)
...
%
% mom-scope change: increase gad threshold
...
for curr_cell = 1:size(pyr_matrix,1);
    ...
    event_start_frames = arrayfun(@(x) find(pyr_matrix(curr_cell,...
        (1:peak_frames(x))) < low_df_thresh(x),1,'last'), ...
        1:length(peak_frames),'uni',false);
