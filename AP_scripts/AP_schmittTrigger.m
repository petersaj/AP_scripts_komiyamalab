...
% trace_std gives the standard deviation of the trace
...
for i = 1+peak_back:length(v)-length(delta)-1;
    ...
    if any(prior_frames_diff > delta(1));
        if all(v(i:i+length(delta)-1) > delta) && ...
            ...
            peak_amplitude = v(peak_frame); %max(max(v(i:i+local_max) - v(i-prior_min)));
            % assume baseline of 0 here
            break
        else
            peak_frame = [];
            peak_amplitude = [];
