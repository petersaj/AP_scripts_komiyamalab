...
for roi_num = 1:size(roi_trace_long,1);
%for roi_num = 47:size(roi_trace_long,1);
...
frame_search = 8; % frames to look past peak
frame_back = 3; % frames to look behind peak
baseline_search = 200; % surrounding frames to estimate local baseline
max_std = 4; % keep events over this std amount, even if bad fit
peak_back = 5; % frames within first trigger has to occur
peakstd = [2 2 1 1 1]; % definitions for max trigger, in stds
...
while curr_point < (size(roi_trace_long,2) - frame_search+1) && ...
    ...
    waitbar(curr_point/size(roi_trace_long,2),w,['Fitting events, ROI ' num2str(curr_roi)]);
end
close(w);
