function [peak_frames peak_amplitudes] = ap_schmittDetect(v, delta, peak_back, local_max, drop_percentage)
...
i = 1+peak_back+pre_baseline_frames;
    ...
    if any(prior_frames_diff >= delta(1));
        if all(v(i:i+length(delta)-1) >= delta+v(i-prior_min)) && ...
            ...
            if drop_flag == 1 %&& downward_artifact == 0;
                ...
                % if the trace didn't go below 20%, just take minimum
