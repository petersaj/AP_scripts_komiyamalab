function roi_trace_df = AP_baselineEstimation(roi_trace_long,framerate)
...
for curr_cell = 1:num_rois;
    ...
    for baseline_est = 1:baseline_est_runs;
        ...
        % 2) Find mode to nearest hundredth of loess smoothed trace and get the
        ...
        norm_fit_obj = gmdistribution.fit(noise_smooth',1);
        dx = (max(noise_smooth')-min(noise_smooth'))/length(noise_smooth');
        x = min(noise_smooth'):dx:max(noise_smooth');
        ...
        % This assumes that the current baseline is already within 3 std of the
        % actual baseline.
        
        % take 3x std above and below as "activity" bounds. if this is one
        % of the preliminary estimates, then only worry about things going
        % over threshold.
        lb = noise_smooth_mode + baseline_smooth -3*noise_std;
        ub = noise_smooth_mode + baseline_smooth + 3*noise_std;
        ...
        active_indx_surround = bound_indx;
    ...
    % with the active parts indexed, fill a vector with only inactive frames
    inactive_baseline_smooth_part = [];
    inactive_baseline_smooth_part = ...
        smooth(noise_smooth(~active_indx_surround), baseline_smooth_frames);
    
    inactive_baseline_smooth = NaN(size(baseline_smooth));
    ...
    final_baseline = inactive_baseline_full+baseline_offset;%+baseline_smooth+baseline_offset;
