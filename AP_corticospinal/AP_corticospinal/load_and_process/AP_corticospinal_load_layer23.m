...
for curr_animal = 1:length(animals)
    ...
    for curr_day = 1:length(days);
        ...
        lever_force_all = cell(length(xsg_filenames),1);
        frame_times_all = cell(length(xsg_filenames),1);
        ...
        for curr_xsg = 1:length(xsg_fullfilenames)
            ...
            % Save full lever position and inferred frame times (to have
            % move/act analysis for all movements)
            lever_force_all{curr_xsg} = xsg_data.data.acquirer.trace_2;                      
            frame_times_all{curr_xsg} = [1:loop_size(curr_xsg)]/framerate;
        ...
        % Store all lever and frame times to get all move/act
        data(curr_animal).bhv(curr_day).lever_force = vertcat(lever_force_all{:});
        
        frame_times_add = [0;cumsum(cellfun(@length,lever_force_all(1:end-1))/xsg_samplerate)];
        frame_times_loop = cellfun(@(x,y) x+y,frame_times_all,num2cell(frame_times_add),'uni',false);              
        data(curr_animal).bhv(curr_day).frame_times = horzcat(frame_times_loop{:})';
