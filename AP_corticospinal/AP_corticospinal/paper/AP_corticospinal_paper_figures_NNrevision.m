...
% stats
[h,p,ks2stat] = kstest2(vertcat(template_corr{:,1:4}),vertcat(template_corr{:,11:14}));
...
for curr_days = 1:length(use_days)
    for curr_bin_idx = 1:length(plot_bins)
        curr_bin = plot_bins(curr_bin_idx);
        subplot(length(use_days),length(plot_bins),length(plot_bins)*(curr_days-1)+curr_bin_idx); 
