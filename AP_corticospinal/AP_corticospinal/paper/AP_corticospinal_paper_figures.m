...
% ylabel('Z-score')
% (NN doesn't allow bar plots)
subplot(1,3,3); hold on;
plot(1:2,m2m_tril','k')
plot(3:4,q2q_tril','k')
ylabel('Classification overlap (z-score)')
set(gca,'XTick',1:4);
set(gca,'XTickLabel',{'M2M week 1','M2M week 2','Q2Q week 1','Q2Q week 2'});
...
across_data_use = across_data(1:end-1,:);
% (slope difference)
curr_fit = arrayfun(@(x) polyfit(1:n_bins,early_data_use(:,x)',1),1:size(early_data_use,2),'uni',false);
early_slope = cellfun(@(x) x(1),curr_fit);

curr_fit = arrayfun(@(x) polyfit(1:n_bins,late_data_use(:,x)',1),1:size(late_data_use,2),'uni',false);
late_slope = cellfun(@(x) x(1),curr_fit);

curr_fit = arrayfun(@(x) polyfit(1:n_bins,across_data_use(:,x)',1),1:size(across_data_use,2),'uni',false);
