...
% This variant is to use known events (after analysis, background
% subtraction, etc) saved somewhere as a variable ca_events which is a 2xN
% matrix containing the onsets and offsets of all events


%% Get file containing known events

[events_file,events_path] = uigetfile('*.mat','Choose file with known events');

load([events_path,events_file]);
...
% Error out if number of files doesn't match days in event matrix
if length(tiff_files) ~= length(ca_events)
    error('TIFF files don''t match known calcium event days')
end
...
% Save user settings
user_settings = struct('border',cell_border,'downsample_px',downsample_px);
...
for curr_session = 1:length(tiff_files)
    ...
    for curr_roi_idx = 1:length(use_rois)
        ...
        % For all known events: get maximum projection, average max
        curr_events = ca_events{curr_session}{curr_roi};
        if ~isempty(curr_events)
            % NOTE! this assumes 50 summed frames and no dropped frames
            curr_events_sumframes = ceil(curr_events./50);
            curr_events_sumframes(:,any(curr_events_sumframes > size(cell_im,2),1)) = [];
            curr_events_sumframes_cell = mat2cell(curr_events_sumframes,2,ones(1,size(curr_events_sumframes,2)));
            curr_act_im = reshape(nanmean(cell2mat(permute(cellfun(@(x) max(cell_im(:,x(1):x(2)),[],2), ...
                curr_events_sumframes_cell,'uni',false),[1 3 2])),3),sum(cell_y_use),sum(cell_x_use),[]);
            ...
            curr_act_im_norm = mat2gray(curr_act_im,[0 max_px]);
            rois(curr_roi_idx).im{curr_session}(cell_y_use,cell_x_use,2) = curr_act_im_norm;
            
        else
            rois(curr_roi_idx).im{curr_session}(:,:,2) = 0;
        end
