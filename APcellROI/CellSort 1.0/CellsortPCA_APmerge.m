    ...
    function [covmat, mov, movm, movtm] = create_xcov(fn, pixw, pixh, useframes, nt, dsamp)
        ...
        if (dsamp_space==1)
           mov = []; %zeros(pixw, pixh, nt);
            for curr_file = 1:length(fn);
                imageinfo=imfinfo(fn{curr_file},'tiff');
                numframes=length(imageinfo);
                %for jjind=1:length(useframes)
                for loadframe = 1:numframes
                    border = 15;
                    frame_no_border = imread(fn{curr_file},loadframe,...
                        'PixelRegion',{[border+1 pixh-border] [border+1 pixw-border]});
                    frame_border = zeros(pixh,pixw);
                    frame_border(border+1:end-border,border+1:end-border) = frame_no_border;
                    mov(:,:,loadframe) = frame_border;
                    if mod(loadframe,500)==1
                        fprintf(' Read frame %4.0f out of %4.0f; ', loadframe, numframes)
                        toc
                    end
                end
            end
            ...
            [pixw_dsamp,pixh_dsamp] = size(imresize( imread(fn{1},1), 1/dsamp_space, 'bilinear' ));
            mov = zeros(pixw_dsamp, pixh_dsamp, nt);
            for jjind=1:length(useframes)
                jj = useframes(jjind);
                mov(:,:,jjind) = imresize( imread(fn{1},jj), 1/dsamp_space, 'bilinear' );
                if mod(jjind,500)==1
